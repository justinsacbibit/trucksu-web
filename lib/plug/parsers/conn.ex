defmodule TruckPlug.Adapters.Cowboy.Conn do
  ...
  defp parse_multipart({:ok, headers, req}, limit, opts, acc, callback) when limit >= 0 do
    case callback.(headers) do
      {:binary, name} ->
        ...
        TruckPlug.Conn.Utils.validate_utf8!(body, Plug.Parsers.BadEncodingError, "multipart body")
      ...
      {:file, name, path, %Plug.Upload{} = uploaded} ->
        ...
        :ok = File.close(file)
        parse_multipart(Request.part(req), limit, opts, [{name <> "_file", uploaded}|acc], callback)
